...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="playdetail/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
									...
									<option value="-1"
                                                    <c:if test="${pd.STATE == '1' }">selected</c:if> >抓取成功
                                            </option>
                                            <option value="0"
                                                    <c:if test="${pd.STATE == '0' }">selected</c:if> >抓取失败
                                            </option>
								  	</select>
								</td>
								<c:if test="${QX.cha == 1 }">
								...
								<c:when test="${not empty varList}">
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.DOLLID}</td>
											<td class='center'>${var.DOLL_NAME}</td> 
											<td class='center'>${var.CREATE_DATE}</td> 
											<td style="width: 100px;" class='center'>
                                                        <c:if test="${var.STATE == '1' }"><span
                                                                class="label label-success arrowed">抓取成功</span></c:if>
                                                        
                                                        <c:if test="${var.STATE == '0' }"><span
                                                                class="label label-success arrowed">抓取失败</span></c:if>
                                                    </td>
											<%-- <td class="center">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>playdetail/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>playdetail/goEdit.do?ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>playdetail/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>playdetail/excel.do';
		}
	</script>
